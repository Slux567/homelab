resource "proxmox_vm_qemu" "kubernetes-worker-1" {

  # -- Basic VM Info
  name        = "kubernetes-worker-1"
  vmid        = 201
  target_node = "midgard"
  desc        = "This VM is used as a worker node in the Kubernetes cluster."
  ...
  ipconfig0   = "ip=10.30.24.231/16,gw=10.30.0.1"  # Change IP as needed
  skip_ipv6   = true
